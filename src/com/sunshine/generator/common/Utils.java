package com.sunshine.generator.common;
...
import com.sunshine.generator.model.Element;
...
//    }
     ...
     * @return
     */
    public static String getStringFromPsiAnnotationMemberValue(PsiAnnotationMemberValue value){
        if(value instanceof PsiReferenceExpression){//常量
            PsiReferenceExpression expression = (PsiReferenceExpression) value;
            String constants = expression.resolve().getText();
            Pattern pattern = Pattern.compile("\"(.*)\"");
            Matcher matcher = pattern.matcher(constants);
            if(matcher.find()){
                return matcher.group(1);
            }
        }else if(value instanceof PsiLiteralExpression){//字符串
    ...
    public static void get(PsiFile file){
        if(file instanceof PsiJavaFile){
            PsiJavaFile javaFile = (PsiJavaFile) file;
            PsiClass[] classed = javaFile.getClasses();
            for (PsiClass clz:classed){
                PsiField[] fields = clz.getFields();
                for (PsiField field:fields){
                    System.out.println("type="+field.getType()+",value="+field.getName());
                }
            }
        }
    }
